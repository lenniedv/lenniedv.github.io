...
function Toolbox() {
    ...
    var addToolIcon = function(icon, name, description) {
        var sideBarItem = createDiv("<img src='" + icon + "' title='" + description + "'></div>")
    ...
    this.addTool = function(tool) {
        //check that the object tool has an icon, name and a description
        if (!tool.hasOwnProperty('icon') ||
            !tool.hasOwnProperty('name') ||
            !tool.hasOwnProperty('description')
        ) {
            alert('make sure your tool has a name, icon and a description')
        }
        this.tools.push(tool)
        addToolIcon(tool.icon, tool.name, tool.description)
    ...
    this.selectTool = function(toolName) {
        ...
        for (var i = 0; i < this.tools.length; i++) {
            if (this.tools[i].name == toolName) {
                ...
                select('#' + toolName + 'sideBarItem').style('border', '2px solid #ECECEC')
